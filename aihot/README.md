...
让支持 Agent Skills（`SKILL.md`）的工具查询 [AI HOT](https://aihot.virxact.com) 当前精选、最近 7 天公开动态、热点和日报，也可低流量维护当前全部精选副本。
...
定时任务只关心最近有没有新内容时，会先检查轻量指纹；需要维护当前全部精选时，会首次取一次快照、之后只取变化。两种协议不会叠加轮询，也不需要反复下载最近 7 天列表。
